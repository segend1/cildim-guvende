...
const NEW_DATA_FILE = path.join(__dirname, '../../data/dermokil_products.json');

function generateId() {
    return 'dermokil-' + Math.random().toString(36).substr(2, 9);
}
...
function merge() {
    ...
    newData.forEach(p => {
        ...
        if (!p.ingredients || p.ingredients.length < 5) { // Adjusted limit for safety
